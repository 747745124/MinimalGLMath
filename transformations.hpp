...
static mat4 translation3D(vec3 translation) {
  return mat4{1, 0, 0, translation[0], 0, 1, 0, translation[1],
              0, 0, 1, translation[2], 0, 0, 0, 1};
};

// 3D scaling matrix given a scaling vector
static mat4 scale3D(vec3 scaling) {
  return mat4{scaling[0], 0, 0,          0, 0, scaling[1], 0, 0,
              0,          0, scaling[2], 0, 0, 0,          0, 1};
};

// 3D perspective matrix given a field of view, aspect ratio, near and far
// planes Renderman is a little bit different (LH) -1  ～ 1 for x & y, but 0 ～
// 1 for z
static mat4 getPerspectiveMatLH(float fov, float aspect,
                                               float hither, float yon) {
  return mat4{1 / tan(fov / 2),
              0,
              0,
              0,
              0,
...
static mat4 getPerspectiveMatRH(float fov, float aspect,
...
static mat4 getOrthoMatRH(float left, float right, float bottom,
                                         float top, float zNear, float zFar) {
  return mat4{2 / (right - left),
              0,
              0,
...
static mat4 getOrthoMatLH(float left, float right, float bottom, float top,
                          float zNear, float zFar) {
  return mat4{2 / (right - left),
              0,
              0,
...
static mat4 getViewMat(vec3 eye, vec3 center, vec3 up) {
  ...
  vec3 u = cross(f, s);
  return mat4{s[0], u[0], f[0], 0, s[1],         u[1],         f[1],         0,
              s[2], u[2], f[2], 0, -dot(s, eye), -dot(u, eye), -dot(f, eye), 1};
};
